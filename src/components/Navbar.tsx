  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-slate-950/80 backdrop-blur-md border-b border-slate-800">
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center space-x-2.5">
            ...
            <div className="relative">
              <button onClick={() => setRoleDropdownOpen(v => !v)} aria-expanded={roleDropdownOpen} aria-haspopup="menu" className="flex items-center space-x-1.5 px-2.5 py-1.5 rounded-lg bg-slate-900 border border-slate-800 hover:border-slate-700 text-xs font-medium text-slate-200">
                ...
                <span className="capitalize hidden sm:inline text-[11px] text-slate-300 font-mono">{userProfile?.role || 'Student'}</span><ChevronDown size={12} className="text-slate-400" />
              </button>
              {roleDropdownOpen && <div className="absolute right-0 mt-2 w-48 rounded-xl bg-slate-900 border border-slate-800 shadow-2xl py-1 z-50 text-xs">
                <div className="px-3 py-2 border-b border-slate-800"><div className="font-semibold text-slate-200">{userProfile?.fullName || 'Student'}</div><div className="text-[11px] text-slate-400 truncate">{userProfile?.email || 'student@netlab.local'}</div></div>
                <button onClick={() => { onOpenAuth(); setRoleDropdownOpen(false); }} className="w-full text-left px-3 py-2 flex items-center gap-2 text-cyan-400 hover:bg-slate-800"><LogIn size={13} /> Sign in / Account</button>
                <div className="px-2 py-1 text-[10px] font-mono uppercase text-slate-500 font-semibold">Demo Role</div>
                {(['student', 'instructor', 'admin'] as UserRole[]).map(r => <button key={r} onClick={() => handleRoleChange(r)} className={`w-full text-left px-3 py-1.5 flex items-center justify-between hover:bg-slate-800 transition ${userProfile?.role === r ? 'text-cyan-400 font-semibold' : 'text-slate-300'}`}><span className="capitalize">{r}</span>{userProfile?.role === r && <span className="w-1.5 h-1.5 rounded-full bg-cyan-400" />}</button>)}
      ...
      {mobileMenuOpen && <div className="xl:hidden border-t border-slate-800 bg-slate-950 px-4 pt-2 pb-6 space-y-1 animate-fadeIn">
        ...
        <button onClick={() => { onOpenAuth(); setMobileMenuOpen(false); }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-cyan-400 hover:bg-slate-900"><LogIn size={18} /><span>Sign in / Account</span></button>
      </div>}
    </header>
  );
};
